...
def decode_array(arg_str, lpc_enabled):
...
def argument_parse_handler():
    text = "this program decodes the SW Part Numbers array readout response from ParallellDiag tool in SPA2\n-paste the array as argument. The script will print out the SW Part Numbers individually in same format as presented in ConfigHub"
    parser = argparse.ArgumentParser(
        prog="readout decoder",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        description=text, epilog="---------------------------")

    parser.add_argument('-lpc', '--lpc', action="store_true", help='set flag only if argument is the response from a LPC SW Part Number readout. If -lpc flag not set, given argument is intepretated HPA SW Part Number')  # add flag without any argument
    ...
    if args.lpc == True:
        ...
        lpc_flag_enabled = False
    #print(arg_str)
    return arg_str, lpc_flag_enabled

def print_intro():
    print("\n//// ECU SW Readout Decoder 0.0.1, creator Oliver Staberg, date 2023-03-23 ////")

def main():
    print_intro()
    sw_array_raw, lpc_enabled = argument_parse_handler()
    decode_array(sw_array_raw, lpc_enabled)
